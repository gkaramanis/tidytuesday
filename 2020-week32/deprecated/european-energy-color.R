...
map <- ggplot(power_lines) +
  ...
  # tower
  geom_segment(data = towers, aes(x = tx, y = ty, xend = txend, yend = tyend), size = 0.8, lineend = "round") +
  geom_segment(data = towers, aes(x = tx + (txend -tx) / 2, y = ty + (tyend - ty) / 2, xend = tx + (txend -tx) / 2, yend = latitude), size = 0.8, lineend = "round") +
  # country names
  ...
  # lines
	geom_segment(aes(x = x, y = y, xend = xend, yend = yend, colour = type), size = 0.3) +
  # annotation
...
inset <- ggplot(subset(power_lines, country == "DE")) +
  ...
  # tower
  geom_segment(data = subset(towers, country == "DE"), aes(x = tx, y = ty, xend = txend, yend = tyend), size = 1, lineend = "round") +
  geom_segment(data = subset(towers, country == "DE"), aes(x = tx + (txend -tx) / 2, y = ty + (tyend - ty) / 2, xend = tx + (txend -tx) / 2, yend = latitude), size = 0.8, lineend = "round") +
  ...
  # lines
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend, colour = type), size = 0.5) +
	# annotations
  annotate("text", 44.5, -34.5, label = "Fossil", size = 4, vjust = 1, hjust = 0, family = "Proxima Nova Bold", colour = pal[1]) +
  annotate("text", 44.5, -35.5, label = "Other", size = 4, vjust = 1, hjust = 0, family = "Proxima Nova Bold", colour = darken(pal[3], 0.3)) +
  annotate("text", 54, -37.5, label = "Reneweable", size = 4, vjust = 1, hjust = 0, family = "Proxima Nova Bold", colour = pal[4]) +
  annotate("text", 54, -38.5, label = "Nuclear", size = 4, vjust = 1, hjust = 0, family = "Proxima Nova Bold", colour = pal[2]) +
...
ggdraw(map) +
  ...
  ggsave(here::here("2020-week32", "plots", "temp", paste0("european-energy-", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 12, height = 12)