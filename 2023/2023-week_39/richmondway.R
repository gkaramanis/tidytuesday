...
ggplot(richmondway) +
  ...
  annotate("shadowtext", -0.5, 1, label = "ROY KENT\n\n\nFUCKS PER\nEPISODE", hjust = 0.5, vjust = 1, family = f2, size = 12, fontface = "bold", lineheight = 0.9, color = colorspace::lighten(col_grey, 0.2), bg.r = 0.04, bg.color = "grey99") +
  annotate("text", -0.5, 5.4, label = "✹\n\nSource:\nrichmondway\nR package\n\nGraphic:\n Georgios Karamanis", hjust = 0.5, vjust = 1, family = f2, size = 5, lineheight = 0.9, color = col_grey) +
  # annotate("point", -0.5, 8, size = 5, color = col_grey) +
  ...
  theme(
    ...
    axis.text = element_text(color = col_grey, size = 9, margin = margin(0, 2, 2, 0), face = "bold"),
    axis.title = element_text(margin = margin(5, 5, 5, 5), size = 9, color = col_grey, face = "bold"),
