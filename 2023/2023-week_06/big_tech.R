...
ggplot(stock) +
  ...
  scale_fill_gradientn(colors = pal,  guide = guide_colorbar(direction = "horizontal"), breaks = seq(0, 700, 100), labels = c(seq(0, 600, 100), "700$"), limits = c(0, 700)) +
  labs(
    title = "Big Tech Stock Prices, 2017–2022",
    caption = "Showing only business days · Source: Yahoo Finance · Graphic: Georgios Karamanis",
    fill = "Closing price after adjustments for all\napplicable splits and dividend distributions"
  ) +
  ...
  theme(
    ...
    legend.title = element_text(vjust = 1, hjust = 1, margin = margin(0, 5, 0, 0), lineheight = 1),
