...
race_anim <- ggplot() +
  # city background 
  geom_rect(aes(xmin = 1, ymin = 0, xmax = 13, ymax = 125), fill = "grey60") +
  # city street
  geom_rect(aes(xmin = 3, ymin = 0, xmax = 8.75, ymax = 125), fill = "grey20") +
  # city street line
  geom_segment(aes(x = 7, y = 0, xend = 7, yend = 125), size = 2, color = "grey90", linetype = 2) +
  geom_segment(aes(x = 5, y = 0, xend = 5, yend = 125), size = 2, color = "grey90", linetype = 2) +
  # city label
  geom_text(aes(x = 4, y = 1, label = "CITY"), size = 16, color = "grey70", alpha = 0.2, family = "IBM Plex Sans Medium", angle = 90, hjust = "left") +
  
  # highway background
  geom_rect(aes(xmin = 14, ymin = 0, xmax = 26, ymax = 125), fill = "brown4") +
  # highway 
  geom_rect(aes(xmin = 18, ymin = 0, xmax = 23.75, ymax = 125), fill = "grey20") +
  # highway road lines
  geom_segment(aes(x = 20, y = 0, xend = 20, yend = 125), size = 2, color = "grey90", linetype = "longdash") +
  geom_segment(aes(x = 22, y = 0, xend = 22, yend = 125), size = 2, color = "grey90", linetype = "longdash") +
  # highway label
  geom_text(aes(x = 19, y = 1, label = "HIGHWAY"), size = 16, color = "grey70", alpha = 0.2, family = "IBM Plex Sans Medium", angle = 90, hjust = "left") +
  
  # city cars
  geom_image(data = city_race, aes(x = lane, y = city_mpg_median, image = car, color = make), size= .05, asp = 0.7) +
  # city cars make
  ...
  # highway cars
  geom_image(data = highway_race, aes(x = lane, y = highway_mpg_median, image = car, color = make), size= .05, asp = 0.7) +
  # highway cars make
  ...
  geom_text(aes(x = 13.5, y = seq(0, 120, 10)), label = seq(0, 120, 10), family = "IBM Plex Mono Light", size = 6, color = "black") +
  geom_text(aes(x = 13.5, y = seq(5, 115, 10)), label = seq(5, 115, 10), family = "IBM Plex Mono Light", size = 6, color = "grey50") +
  ...
  animate(
    race_anim, fps = 2, detail = 5,
    start_pause = 5, end_pause = 15,
    width = 760, height = 1200,
    renderer = gifski_renderer(here::here("week-42", "figures", "race.gif"))
  )
